...
def compute_entropy_with_cov(log_prob, advantages, response_mask, entropy, loss_agg_mode="token-mean", clip_cov_ub=5.0, cov_alpha=10.0):

    cov_all = (advantages- verl_F.masked_mean(advantages, response_mask)) * (log_prob- verl_F.masked_mean(log_prob.detach(), response_mask))
    cov_all[response_mask == 0] = -torch.inf

    entropy_coeff_token_level = torch.nn.functional.softmax(cov_all, dim=-1)
    response_length = entropy_coeff_token_level.shape[-1]
    entropy_coeff_token_level = entropy_coeff_token_level * cov_alpha
    entropy_coeff_token_level = torch.clamp(entropy_coeff_token_level, 0.0, clip_cov_ub)
    entropy_loss = agg_loss(
        loss_mat=entropy * entropy_coeff_token_level,
        loss_mask=response_mask,
        loss_agg_mode=loss_agg_mode
    )
    entropy_coeff_token_level_avg = agg_loss(
        loss_mat=entropy_coeff_token_level,
        loss_mask=response_mask,
        loss_agg_mode=loss_agg_mode
    )
    return entropy_loss, entropy_coeff_token_level_avg
